    ...
    _createClass(World, [{
      ...
      value: function makeGrid() {
        ...
        while (i < this.rows) {
          ...
          var j = 0;
          while (j < this.cols) {
            var cell = new App.Cell(false, j, i);
      ...
      key: 'withinYMidRange',
      value: function withinYMidRange(yCoord, spaceshipHeight) {
        var min = this.yMidpoint - spaceshipHeight;
        var max = this.yMidpoint + spaceshipHeight;
        return yCoord >= min && yCoord <= max;
      }
    }, {
      key: 'addXCushion',
      value: function addXCushion(startPoint, direction) {
        return direction === 'easterly' && startPoint[0] >= this.xMidpoint || direction === 'westerly' && startPoint[0] <= this.xMidpoint;
      }
    }, {
      key: 'getXYOffset',
      value: function getXYOffset(startPoint, direction) {
        var XYOffset = [0, 0];
        var addXCushion = this.addXCushion(startPoint, direction);
        switch (this.pattern) {
          case "glider":
            XYOffset = [5, 5];
            if (addXCushion) {
      ...
      key: 'calcPattern',
      value: function calcPattern(startPoint) {
        var cells = [];
        var cell = this.cellGrid[startPoint[1]][startPoint[0]];
        var xOp = this.operators[cell.x < this.xMidpoint ? '+' : '-'];
        var yOp = this.operators[cell.y < this.yMidpoint ? '+' : '-'];
        switch (this.pattern) {
          case "glider":
            if (cell.x > 1 && cell.x < this.cols - 2 && cell.y > 1 && cell.y < this.rows - 2) {
              cells.push.apply(cells, [cell, this.cellGrid[yOp(cell.y, 2)][cell.x], // north 1
          ...
          case "lightweight":
            if (cell.x > 3 && cell.y > 2) {
              cells.push.apply(cells, [cell, this.cellGrid[yOp(cell.y, 2)][cell.x], this.cellGrid[yOp(cell.y, 1)][cell.x], this.cellGrid[cell.y][xOp(cell.x, 1)], this.cellGrid[cell.y][xOp(cell.x, 2)], this.cellGrid[cell.y][xOp(cell.x, 3)], this.cellGrid[yOp(cell.y, 1)][xOp(cell.x, 4)], this.cellGrid[yOp(cell.y, 3)][xOp(cell.x, 4)], this.cellGrid[yOp(cell.y, 3)][xOp(cell.x, 1)]]);
            }
            break;
          case "heavyweight":
            if (cell.x > 3 && cell.y > 3 && cell.y < this.rows - 1) {
              cells.push.apply(cells, [cell, this.cellGrid[cell.y][xOp(cell.x, 1)], this.cellGrid[cell.y][xOp(cell.x, 2)], this.cellGrid[cell.y][xOp(cell.x, 3)], this.cellGrid[cell.y][xOp(cell.x, 4)], this.cellGrid[cell.y][xOp(cell.x, 5)], this.cellGrid[yOp(cell.y, 1)][xOp(cell.x, 6)], this.cellGrid[yOp(cell.y, 3)][xOp(cell.x, 6)], this.cellGrid[yOp(cell.y, 4)][xOp(cell.x, 4)], this.cellGrid[yOp(cell.y, 4)][xOp(cell.x, 3)], this.cellGrid[yOp(cell.y, 3)][xOp(cell.x, 1)], this.cellGrid[yOp(cell.y, 2)][cell.x], this.cellGrid[yOp(cell.y, 1)][cell.x]]);
            }
            break;
          default:
            cells.push(cell);
      ...
      key: 'randomlyPopulate',
      value: function randomlyPopulate() {
        this.cells.forEach(function (cell) {
...
$(function () {
  var Game = (function () {
    function Game() {
      ...
      this.callback;
      this.callbacks = { 'tick': this.callBack = function () {
          return game.tick();
        } };
    ...
    _createClass(Game, [{
      ...
      value: function resize() {
        ...
        var controlsWidth = $('.controls').css("width").match(/\d+/g)[0];
        var rightMargin = $('.controls').css("margin-right").match(/\d+/g)[0];
        var widthOffset = parseInt(controlsWidth) + parseInt(rightMargin) + 50;
      ...
      key: 'roundDownToTen',
      value: function roundDownToTen(num) {
        return Math.floor((num + 1) / game.colWidth) * game.colWidth;
      }
    }, {
      key: 'handlePatternSelect',
      value: function handlePatternSelect() {
        var name = $("input[type=radio]:checked").val();
        game.world.pattern = name;
      }
    }, {
      key: 'withinYRange',
      value: function withinYRange(currentY, previousY) {
        if (currentY === previousY) {
          return true;
        }
        var min = previousY - 8; // large spaceship height
        var max = previousY + 8;
        return currentY >= min && currentY <= max;
      }
    }, {
      key: 'getDirection',
      value: function getDirection(startPoint, previousStartPoint) {
        var currentX = startPoint[0];
        var previousX = previousStartPoint[0];
        var currentY = startPoint[1];
        var previousY = previousStartPoint[1];
        var direction = null;
        var yIsInRange = game.withinYRange(currentY, previousY);
      ...
      value: function drawPattern() {
        if (game.enableDraw === false) {
          return;
        }

        var startPoint = game.getStartPoint();
        var xDiff = Math.abs(startPoint[0] - game.previousStartPoint[0]);
        var yDiff = Math.abs(startPoint[1] - game.previousStartPoint[1]);
        var direction = game.getDirection(startPoint, game.previousStartPoint);

        var XYOffset = game.world.getXYOffset(startPoint, direction);

        if (xDiff >= XYOffset[0] || yDiff >= XYOffset[1]) {
          game.previousStartPoint = startPoint;
          var cells = game.world.calcPattern(startPoint);
      ...
      value: function getStartPoint() {
        ...
        var x = this.normalizeCoord(event.clientX - rect.left);
        var y = this.normalizeCoord(event.clientY - rect.top);
        console.log("x: " + x + " y: " + y);
        return [x, y];
      }
    }, {
      key: 'normalizeCoord',
      value: function normalizeCoord(coord) {
        return coord < 10 ? 0 : Math.floor(coord / 10);
      }
    }, {
      key: 'handleGameToggle',
      value: function handleGameToggle() {
        var value = $(this).html();
      ...
      value: function tick() {
        ...
        var liveCellsNextRound = [];
        var deadCellsNextRound = [];
        this.world.cells.forEach(function (cell) {
          var neighbors = _this.world.liveNeighbors(cell).length;
        ...
        liveCellsNextRound.forEach(function (cell) {
          cell.alive = true;
        });
        deadCellsNextRound.forEach(function (cell) {
          cell.alive = false;
        });
        this.revolutions++;
      ...
      value: function drawCell(cell) {
        ...
        this.ctx.fillRect(cell.x * this.colWidth, cell.y * this.rowHeight, this.colWidth - 1, this.rowHeight - 1);
      }
    }]);
